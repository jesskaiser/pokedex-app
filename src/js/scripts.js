...
let pokemonRepository = (function () {
    ...
    function showModal(pokemon) {
        ...
        let heightElement = $(
            `<p><span class="text-style">Height</span>: ${pokemon.height}</p>`
        )
        let weightElement = $(
            `<p><span class="text-style">Weight</span>: ${pokemon.weight}</p>`
        )

        //Pokemon type
        let typeElement = $(
            `<p><span class="text-style">${`Type(s)</span>: ${pokemon.types[0].type['name']}`}</p>`
        )
        ...
        //Abilities
        let abilityElement = $(
            `<p><span class="text-style">${`Abilities</span>: ${pokemon.abilities[0].ability['name']}`}</p>`
        )
        ...
        bodyElement.attr(
            'class',
            'modal-body justify-content-between align-items-center row'
        )
...
let findPokemon = function (name) {
    ...
    $('.list-group-item button').filter(function () {
        $(this)
            .closest('div')
            .toggle($(this).text().toLowerCase().indexOf(name) > -1)
    })
}
